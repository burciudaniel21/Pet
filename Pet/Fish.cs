...
namespace Pet
{
    class Fish : IPet //This class follows Liskov substitution principle as it is perfectly substitutable for the IPet class.
    {
        private string name;
        ...
        private int hunger;
        private int preferredTemperautre = 26;
...
|_/     \_ (   (    {UpdatePetVisual()}__/
   ...
   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        ...
        {
            return preferredTemperautre;
        }

        public override void UpdateByRoomTemperature() //if room temperature is too high or too low, pet HP is affected
        {
            while(GetTemperature() > preferredTemperautre + 5 || GetTemperature() < preferredTemperautre - 5 && petHp > 0) 
            {
                petHp--;
                Thread.Sleep(1500);
            }
